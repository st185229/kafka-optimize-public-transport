        ...
        try:
            ...
            if msg is None:
                ...
                if msg.error() is None:
                    ...
                    error = msg.error()
                    logger.error(
                        f"Error in consumer:{self.topic_name_pattern} while consuming msgs. Err code: {error.code()}, "
                        f"error-name: {error.name()}, error.str:{error.str()}")
                return 1
        except RuntimeError as er:
            logger.error(f"Runtime error in consumer:{self.topic_name_pattern}")
